...
- 设置页目前包含页面名称、布局调试和编辑器模式。它们使用“文本 + info 提示图标 +
  输入框/下拉菜单”的形式；页面名称暂存于浏览器本地并同步到网页标题，未来再通过
  后端写入 `tokens.css`；布局调试和编辑器模式替代早期页面右下角的浮动按钮。设置页面
  自身不显示“设置”标题，以保持侧边栏紧凑。
...
切换后页面会重新渲染新建日记区域，并自动回到“现在”位置。状态同样保存在
`localStorage` 中。也可以在 F12 Console 中运行：
