...
进入 P5B 真实写入闭环后，正式新建日记路径固定为离线 `Textarea`：

- backend 数据源下始终使用原生 `<textarea>`，不会读取本地 Tiptap 偏好。
- backend 数据源下不会动态加载 `esm.sh` 或任何 Tiptap CDN。
- P5B 保存时读取的正文必须来自 Textarea Markdown，而不是 Tiptap JSON 或 HTML。
- 字体、字号、颜色等表现型编辑功能不进入当前正式写作范围。

`Tiptap demo` 只保留在 `mock` 数据源中，用于后续重新评估本地化富文本编辑体验。
如果未来要正式接入 Tiptap，需要另开“本地打包/Markdown 适配/媒体引用/无障碍”的任务，
且 `content.md` 仍必须是语义 Markdown。

## 使用方式

在 `mock` 数据源中，设置页可切换编辑器模式：

- `Textarea`：默认稳定模式，使用原生 `<textarea>`。
- `Tiptap demo`：实验模式，使用 Tiptap 渲染新建正文区域。

切换状态会保存到当前浏览器的 `localStorage`，刷新后保留。切换时页面会重新渲染并
定位到新建日记区。该偏好在 backend 数据源下会被忽略。

`Tiptap demo` 模式下会显示一排临时工具栏，用于测试段落、二/三级标题、粗体、斜体、
删除线、行内代码、列表、引用、代码块、撤销和重做。工具栏按钮暂不进入 Tab 顺序，
以免从标题输入框跳转到正文前被按钮打断；正式键盘可达性需要另行设计。

新建区的 DOM 顺序刻意保持为“标题 → 工具栏 → 正文 → 状态消息 → 操作按钮”，
并通过 CSS 将操作按钮视觉放回标题右侧。因此从标题按 Tab 会优先进入正文编辑区，
而不是先经过保存草稿、取消和保存按钮。
