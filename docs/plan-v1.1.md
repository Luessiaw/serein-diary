...
3. **Step 2 — 条目状态：** 使用静态样本列表开发两种状态：已封存阅读，以及未保存
   ...
   并停止继续请求。折叠年份或月份后，如果当前可见内容变短并再次进入顶部触发区，
   前端会继续补拉取更早内容；真实 API 阶段必须根据服务端分页和 `has_more`
   重新判断，而不能假设当前折叠组内已包含完整历史。
...
- 日记列表 API 负责以创建时间正序提供游标分页；前端仅将更早页面插入当前内容
  ...
  并停止继续拉取。折叠或展开导致布局高度变化后，前端需要重新评估是否仍处于
  拉取触发区；如果处于触发区且 `has_more` 为真，应继续请求更早页面。
