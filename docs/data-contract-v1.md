...
```text
DIARY_DATA_DIR/.serein/index.sqlite3
```

`.serein/` 下内容是运行时衍生物，不属于 v1 事实文件。
...
迁移 dry-run 是只读检查，只面向当前 v1 条目目录或外部 v1 条目目录：

- 不复制文件。
- 不删除文件。
- 不修改源目录或目标目录。
- 不打印日记正文。
- 不转换早期 `entries/<year>/...` 等旧结构；此类目录会作为非法目录报告。

报告应包含：

- 条目数量、评论数量、媒体数量。
- 缺失字段、非法目录名、旧字段拒绝结果和时间不一致问题。
- 可自动迁移与需要人工处理的问题清单。
