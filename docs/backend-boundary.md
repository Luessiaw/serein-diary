...
读取配置、暴露版本化 API、完成单管理员访问保护，并通过 Compose/Caddy 示例连通。
...
- 未认证可访问的健康检查接口。
- 极简锁屏风格的单管理员登录、登出、当前会话检查。
- HttpOnly Cookie 会话，使用 `DIARY_SESSION_SECRET` 签名或加密。
- 可复用的访问保护依赖，用于保护后续写入 API。
- 一个最小受保护接口，用于验证访问保护链路。
- Compose 中的 `api` 服务。
...
│   ├── security.py          # 锁屏密码校验、会话签名、Cookie 工具
│   └── api/
│       ├── __init__.py
...
- `/api/v1/health` 不需要认证。
- 访问保护失败返回 `401`，不得泄露密码是否存在、secret、数据目录或日记内容。
- 输入正确管理员密码后，后端设置 HttpOnly Cookie。
- 登出清除 Cookie。
...
打开 Serein
  ↓
前端请求 GET /api/v1/auth/session
  ↓
已有有效 Cookie：直接进入日记页面
没有有效 Cookie：显示极简锁屏
  ↓
输入管理员密码
  ↓
POST /api/v1/auth/login
  ↓
密码正确：设置 HttpOnly Cookie，进入日记页面
密码错误：保持锁屏并显示轻量错误提示
```

锁屏界面应保持极简，只需要应用名、一个密码输入框和一个进入按钮。它不显示用户名、
注册入口、找回密码、权限说明或复杂表单。用户通常只在第一次访问、Cookie 过期、
主动登出、清除 Cookie 或更换 `DIARY_SESSION_SECRET` 后再次输入密码。
...
- 单管理员访问保护是应用自身边界；Tailscale、Caddy basic auth 或其他反向代理认证
  只能作为额外保护，不能替代应用自己的锁屏。
